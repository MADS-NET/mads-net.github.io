...
* As of version 4.2.x, MADS broker sustains up to **~460 clients** (agents) on a single machine on Linux
* More can be supported by setting `RLIMIT_NOFILE` to a higher value (will be done by `mads.ini` in future releases)
...
:::aside
Use the command `ulimit -n <value>` on the broker machine to increase the maximum number of connections. Each agent needs two permamnent sockets, plus a transient one when receiving settings. Linux default is 1024, which is enough for ~500 agents
:::
...
Exercise: [here](gorini_exercise.qmd){target="_blank"}