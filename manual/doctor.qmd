...
abstract: |
  `mads doctor` (`mads-doctor`) is a tool designed to help users spot possible 
  issues and solutions they may encounter when setting up a network of MADS agents.
---
...
digraph mads {
  ...
  arduino    [label="arduino", color=darkred];
  ble_filter [label="ble_filter|{ble_scan\l}", color=darkgreen];
  ble_scan   [label="ble_scan", color=darkred];
  feedback   [label="feedback|{ble_filter\l}", color=darkblue];
  logger     [label="logger|{ble_scan\lble_filter\l}", color=darkblue];
  rerunner   [label="rerunner|{ble_filter\larduino\l}", color=darkblue];

  arduino    -> rerunner [label="arduino"];
  ble_filter -> feedback [label="ble_filter"];
  ble_filter -> logger [label="ble_filter"];
  ble_filter -> rerunner [label="ble_filter"];
  ble_scan   -> ble_filter [label="ble_scan"];
  ble_scan   -> logger [label="ble_scan"];
}
```
