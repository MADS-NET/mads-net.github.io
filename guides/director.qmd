...
:::aside
The director app is also a [standalone project](https://github.com/mads-net/mads_director){target="director"} that can be [installed](https://github.com/MADS-NET/mads_director/releases){target="director"} without Mads. For this reason, the template TOML file contains example commands that are not related to MADS, but you can replace them with any command you want, including `mads` commands.
...
* `enabled`: if false, the command won't be started automatically (but it can be started manually from the GUI)
...
* on the right, form top to bottom: 
  ...
  - launch command
  - buttons for starting, stopping, relaunching, or sending `SIGINFO` to the selected process
