...
To quickly test the plugin, follow the next steps:
...
5. From the `rerunner_plugin` folder, run the `replay` plugin, so that it uses the `example.cvs` file *from the current folder*:
