...
#     toc: true
draft: false
categories: [v2, changes, plugin, crypto]
abstract: |
...
With MADS v2.0, the standard MADS installer provides both a static and a shared library, plus a host of C++ header files. The libraries also provide a pure C wrapper interface (and its corresponding header `agent_c.h`). This makes it easier for:
...
* **delete** the directories `build/plugin-*` (much faster than deleting the whole `build` folder); this forces CMake to re-download the updated version
...
```sh
$(mads -p)/python/update_plugin_source.py src/my_plugin.cpp
```

* update the `CMakeLists.txt` using the same command:

```sh
$(mads -p)/python/update_plugin_source.py CMakeLists.txt
```
