...
MADS follows [Semantic Versioning](https://semver.org). The version number is in the form `MAJOR.MINOR.PATCH`, where:
...
Suppose that you downloaded the installer `mads-<version>-Darwin-universal.sh` (where `<version>` is the version number, e.g. `2.0.0`). The installer takes the following arguments:

```bash
./Mads-v2.4.2-Darwin-universal.sh --help
Usage: packages/Mads-v2.4.2-Darwin-universal.sh [options]
Options: [defaults in brackets after descriptions]
  --help            print this message
  --version         print cmake installer version
  --prefix=dir      directory in which to install
  --include-subdir  include the Mads-v2.4.2-Darwin-universal subdirectory
  --exclude-subdir  exclude the Mads-v2.4.2-Darwin-universal subdirectory
...
```bash
./Mads-v2.4.2-Darwin-universal.sh --prefix=${HOME} --skip-license    
```
...
```bash
export MV=v2.4.2  # or the latest version
...
Since MADS v2.0.0, a Python interface is provided as a module `mads_agent` by the file in `$(mads -p)/python/mads_agent.py`. 

To be able to use it, you need to add the folder `$(mads -p)/python` to the Python search path. There are two possible ways:

**Method 1**: environment variable (probably the easiest way):
```sh
export PYTHONPATH=$PYTHONPATH:$(mads -p)/python
```

If you want to make this permanent, add the above line to your `.zshrc` or `.bashrc` file (on Unix) or to your system environment variables (on Windows).
...
```bash
git clone --depth 1 https://github.com/pbosetti/MADS.git
cd MADS
```
