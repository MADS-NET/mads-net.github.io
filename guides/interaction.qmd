...
<script src="https://asciinema.org/a/jjbmwYcZcPyTZiyz5W0UsKnae.js" id="asciicast-jjbmwYcZcPyTZiyz5W0UsKnae" async="true" data-idle-time-limit=1></script>

::: callout-important
Remember: if the broker runs on another machine, you need to specify its **settings address** with the `-s` option (see [this guide](structure.html#network-layout)).
:::
...
if len(sys.argv) > 1:
  process = subprocess.Popen(['mads', 'bridge', '-t', sys.argv[1]], stdin=subprocess.PIPE)
else:
...
<script src="https://asciinema.org/a/Tk5HreU9d3zQhF9N5p2H5hfsp.js" id="asciicast-Tk5HreU9d3zQhF9N5p2H5hfsp" data-idle-time-limit=1
  async="true"></script>
